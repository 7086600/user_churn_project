...
SELECT
  STRFTIME ('%m', subscription_start) AS 'month',
  COUNT(*)
FROM
  subscriptions
GROUP by
...
-- #9 generalizing the CASE statements and using GROUP BY segment
-- months temp table
WITH
  months AS (
    SELECT
  ...
  status AS (
    SELECT
      id,
      STRFTIME ('%Y_%m', first_day) AS 'Year_month',
      segment,
